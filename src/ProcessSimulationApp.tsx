...
export default function ProcessSimulationApp() {
  ...
  const [resources, setResources] = useState<ProcessResource[]>(initial.resources);
  const [requirementsByBlock, setRequirementsByBlock] = useState<Record<string, ProcessResourceRequirement[]>>(initial.requirementsByBlock);
  ...
  const removeResource = (id: string) => {
    ...
    setRequirementsByBlock(current => {
      const next: Record<string, ProcessResourceRequirement[]> = {};
      for (const blockId of Object.keys(current)) {
        next[blockId] = current[blockId].filter(requirement => requirement.resourceId !== id);
      }
