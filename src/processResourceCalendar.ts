...
function nextWorkingStart(
  ...
  // A task must fit entirely inside one working window. Search bounded number of cycles;
  // this is deterministic and avoids silently spanning a closed shift boundary.
  for (let guard = 0; guard < 10000; guard += 1) {
    const cycleIndex = Math.floor(candidate / cycle);
    const cycleBase = cycleIndex * cycle;
    const offset = candidate - cycleBase;
    ...
    for (const window of windows) {
      ...
      if (offset < window.startOffsetSeconds && windowStart + duration <= windowEnd) return { start: windowStart, shifted: true };
    }
...
export function availableSecondsWithin(
  ...
  for (const downtime of normalizeDowntime(policy)) {
    ...
    // Downtime subtraction is intentionally conservative: subtract only overlap with working windows.
    for (let index = 0; index < cycles; index += 1) {
      const base = index * cycle;
      for (const window of windows) {
